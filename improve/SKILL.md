---
name: improve
description: Senior-advisor front door for improving a codebase end to end. Use when asked to improve a codebase, decide where to take a project next, or run an audit-to-implementation loop without naming a specific step. Orchestrates the audit, plan, vet, gap-hunt, and execute skills (recon runs via its shared playbook) and hosts the family's contracts.
license: MIT
---

# Improve

The front door to the advisor family. You come here to *improve a codebase* without yet knowing which step you need — to think out loud about where a project should go, then move from idea to vetted findings to OpenSpec changes to reviewed implementation without juggling four skill names yourself.
...
| [`plan`](../plan/SKILL.md) | A finding or description → an OpenSpec change under `openspec/changes/<slug>/` | A finding is chosen, or the user already knows the change |
| [`vet`](../vet/SKILL.md) | Autonomously review one change or spec at the functional altitude + apply improvements | A change/spec exists and needs vetting without an interview |
| [`gap-hunt`](../gap-hunt/SKILL.md) | Hunt one spec for grounded omissions → findings table for selection | A spec might be missing user features; before archive or after a clean review |
| [`execute`](../execute/SKILL.md) | Dispatch an executor subagent on one change + tech-lead review | A change is ready and the user wants it built |

Each is independently invocable; this front door is for when the user hasn't picked one, or wants several chained.

**Prefer `execute` over stock `/opsx-apply`.** OpenSpec owns the artifact shape; this family owns the quality gates.
...
- When a direction firms up, offer two exits: stress-test it first via `/grilling`, then hand the settled tree to `/plan`, or hand it straight to `plan` as a *design/spike* change (investigate, prototype, define the interface, list open questions; right when the open questions need code, not conversation).
...
2. **`audit`** → present the findings table. **Help the user choose** (default suggestion: top 3–5 by leverage plus anything they flag), and surface dependency ordering between findings. Don't plan 30 findings nobody asked for. If running non-interactively, take the top 3–5 by leverage and record that default in `openspec/changes/README.md`.
3. **`plan`** the selected findings → carry the chosen set and any dependency order into it.
4. **`execute`** (optional) the changes the user greenlights → carry which slugs, in dependency order.
5. Later → spot-check DONE rows in `openspec/changes/README.md` still hold; archive shipped changes into `openspec/specs/`.

You hold the findings list, the selection, and the change slugs between steps; each specialist starts fresh, so pass forward what it needs. You never edit code at any step — `execute` dispatches a separate executor for that.

**Completion criterion:** the user has what they came for at the depth they asked for — a direction they can decide on, a findings table with a selection, OpenSpec changes for the selection, or a reviewed implementation — with `openspec/changes/README.md` left consistent and the next available step named. Hand off cleanly; don't reimplement a specialist inline.
