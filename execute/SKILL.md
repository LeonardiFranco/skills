---
name: execute
description: Dispatch a cheaper executor subagent to implement one handoff plan, then review its diff like a tech lead and render a verdict. Use when asked to execute or implement a plan from `.scratch/`. The advisor never edits code itself; a separate executor does, and you review.
license: MIT
---
...
**First, read [the advisor contract](../improve/references/advisor-contract.md).** The founding rule survives here unchanged: *the advisor never edits source code.* Running verification commands inside the executor's disposable worktree is fine — the no-mutating-commands rule protects the user's working tree, not the worktree. The same boundary applies to the publish tail (see Verdict): committing and pushing the *worktree's own branch* never touches the user's working tree or branches, so it is within the contract; the user's branches and the default branch remain untouchable.
...
| Verdict     | When                                                                     | Action                                                                                                                                                                                                               |
| ----------- | ------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **APPROVE** | Criteria pass, scope clean, quality holds                                | **Edit the plan tree's** `README.md` — set this plan's row to `DONE`. Then present to the user: diff summary, the worktree path and branch, plus anything from NOTES. **Publish tail (opt-in):** if local steering has a "publish on approve" section (project's local agent config, e.g. `docs/agents/local/preferences.md`), then after the index update: (1) derive the branch name — take the plan filename's slug (the part after the leading `NNN-`/`NN-`, without `.md`), prefix it by the plan's Category (`bug`/`security` → `fixes`, `direction` → `features`, `tech-debt` → `refactors`, else `chores`), as `{prefix}/{slug}`; on collision append `-2`, `-3`, …; (2) in the worktree, commit the executor's changes with a conventional `<type>: <subject>` message on that branch; (3) push to origin; (4) run **`write-pr`** against the pushed branch; (5) publish a **draft** PR with the written description, targeting the default branch — the host commands and any description conventions (length caps, sections posted as comments) come from the project's local agent config (PR host section) and steering, never from this skill; report the PR URL. Any step failing: stop and report exactly what completed (e.g. "branch pushed, PR not created — run: az …") — the verdict stands and the tail is retryable. Never publish on REVISE/BLOCK; never flip a PR out of draft. Without the opt-in, offer **`write-pr`** for a PR description file. **Merging is the user's call.**                         |
